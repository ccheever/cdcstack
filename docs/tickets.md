...
A ticket is a markdown file. Open tickets live in `issues/YYYYMMDD-slug.md`. Closed ones
...
Filesystem tickets are in the diff, greppable, editable by anyone working the repo —
...
cross-repository views. **When a ticket grows into work that needs those, graduate it** —
...
**Do not close a ticket with a bare `git mv`.** The closed path requires `Status: Closed`
...
rather than adding a sixth — it is fast, deterministic, and reports every malformed ticket
...
sprint, no automation that moves tickets between states on your behalf. Those are what a
tracker is for, and reaching for one is a signal that this ticket has graduated.
